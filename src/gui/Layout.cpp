...
void Layout::layoutPages()
{
	...
	int totalWidth = XOURNAL_PADDING;
	for (int c = 0; c < this->columns; c++)
	{
		totalWidth += this->sizeCol[c] + XOURNAL_PADDING_BETWEEN;
		this->sizeCol[c] = totalWidth;	//accumulated for use by getViewAt()
	}
	totalWidth += XOURNAL_PADDING - XOURNAL_PADDING_BETWEEN;

	int totalHeight = XOURNAL_PADDING;
	for (int r = 0; r < this->rows; r++)
	{
		totalHeight += this->sizeRow[r]+ XOURNAL_PADDING_BETWEEN;
		this->sizeRow[r] = totalHeight; //accumulated for use by getViewAt()
	}
	totalHeight += XOURNAL_PADDING - XOURNAL_PADDING_BETWEEN;
