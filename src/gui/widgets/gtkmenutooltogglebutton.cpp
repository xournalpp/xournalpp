...
static void gtk_menu_tool_toggle_button_class_init(GtkMenuToolToggleButtonClass *klass) {
	...
	signals[SHOW_MENU] = g_signal_new("show-menu", G_OBJECT_CLASS_TYPE (klass), G_SIGNAL_RUN_FIRST,
			G_STRUCT_OFFSET (GtkMenuToolToggleButtonClass, show_menu), NULL, NULL, g_cclosure_marshal_VOID__VOID,
			G_TYPE_NONE, 0);
...
static void menu_position_func(GtkMenu *menu, int *x, int *y, gboolean *push_in, GtkMenuToolToggleButton *button) {
	...
	if (orientation == GTK_ORIENTATION_HORIZONTAL) {
		...
		*y += widget->allocation.y;
		...
		if (*y + menu_req.height > monitor.y + monitor.height && *y + priv->arrow_button->allocation.height - monitor.y
...
static gboolean arrow_button_button_press_event_cb(GtkWidget *widget, GdkEventButton *event,
