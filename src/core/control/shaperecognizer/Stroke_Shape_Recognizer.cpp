...
:start_ind{segment.start()}
,end_ind{next_segment.end()}
{
...
Point StrokeSegment::intersect(const StrokeSegment& other) const
{
    ...
    const double t_other{(h.x*main_axis.y-h.y*main_axis.x)/(other.main_axis.x*main_axis.y-other.main_axis.y*main_axis.x)};
...
inline bool StrokeSegment::is_line(double max_minor_is_line) const
{
    return minor < max_minor_is_line;
}

inline Point StrokeSegment::mean() const
{
    return center;
}

inline Point StrokeSegment::axis() const
{
    return main_axis;
}

inline size_t StrokeSegment::start() const
{
    return start_ind;
}

inline size_t StrokeSegment::end() const
{
    return end_ind;
}


Recognizer::Recognizer(RecognizerParameter parameter)
:parameter(std::move(parameter))
{}

inline void Recognizer::set_parameter(RecognizerParameter parameter)
{
...
bool Recognizer::is_polygon()
{
    ...
    size_t segment_size_factor{std::max(size_t(double(nb_points)/double(parameter.min_nb_points_per_segment*parameter.initial_nb_segments)),size_t(1))};
...
void Recognizer::split_into_segments(const std::vector<Point>& points, size_t start, size_t end, size_t segment_size_factor)
{
...
bool Recognizer::fuse_split_final()
{
    ...
    while(working_ind < end)
    {
        ...
        else
        {
            ...
            {
                StrokeSegment fused_prev(prev_segment,StrokeSegment(stroke->getPointVector(),segment.start(),split_ind));
                StrokeSegment fused_next(StrokeSegment(stroke->getPointVector(),split_ind,segment.end()),next_segment);
                if(fused_prev.is_line(parameter.max_minor_is_line) && fused_next.is_line(parameter.max_minor_is_line))
                {
...
bool Recognizer::stabilize_parallelogram()
{
    ...
    if(std::abs(side_length[2]-side_length[0])/side_length[0] < parameter.regularize_rectangle_rel_length_deviation 
    && std::abs(side_length[3]-side_length[1])/side_length[1] < parameter.regularize_rectangle_rel_length_deviation)
    {
        ...
        each case:
            ...
            - use local optimization (initial configuration from current polygon, derivative is easy (der. free might work too)) on objective(free_params_xx) = sum_of_square_distance(polygon,hypothesis_xx(free_params_xx))
        */
        return true;
    }
    return false;
...
bool Recognizer::regularize_polygon()
{
    ...
    for(size_t ind{0}; ind < polygon.size(); ++ind)
    {
        ...
        is_regular_radius &= std::abs((polar[ind].r - mean_radius)/mean_radius) < parameter.regularize_rel_radius_deviation;
    }
    ...
    if(is_regular_angle && is_regular_radius)
    {
        ...
        for(size_t ind{0}; ind < polar.size(); ++ind)
            polygon.push_back(Point(center.x+polar[ind].r*std::cos(polar[ind].phi),center.y+polar[ind].r*std::sin(polar[ind].phi)));
        return true;
    }
    return false;
}