...
void ShapeRecognizer::reduceVerticesToFour(std::vector<Corner>& cornerList) {
    while (cornerList.size() > 5) {
        size_t remove = 0;
        double min = std::numeric_limits<double>::max();
        // We iterate starting from index 1 and stop before the last index
        // The first and last corners are the endpoints of the stroke and
        // must be preserved to maintain the shape's boundaries
        for (size_t x = 1; x < cornerList.size() - 1; x++) {
            if (cornerList[x].dmax < min) {
                min = cornerList[x].dmax;
                remove = x;
            }
        }
        if (remove != 0) {
            cornerList.erase(cornerList.begin() + remove);
        } else {
            break;
        }
    }
