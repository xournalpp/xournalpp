...
XojOpenDlg::XojOpenDlg(GtkWindow* win, Settings* settings): win(win), settings(settings) {
    ...
    if (!settings->getLastOpenPath().empty()) {
        ...
        currentFolder = Util::fromGFilenameUnchecked(g_get_home_dir());
    }
...
auto XojOpenDlg::showOpenDialog(bool pdf, bool& attachPdf) -> fs::path {
    ...
    if (!lastSavePath.empty()) {
        gtk_file_chooser_add_shortcut_folder(GTK_FILE_CHOOSER(this->dialog), Util::toGFilename(lastSavePath).c_str(),
                                             nullptr);
    }
...
void XojOpenDlg::updatePreviewCallback(GtkFileChooser* fileChooser, void* userData) {
    auto filepath = Util::fromGFilename(
            Util::OwnedGFilename::assumeOwnership(gtk_file_chooser_get_preview_filename(fileChooser)));
