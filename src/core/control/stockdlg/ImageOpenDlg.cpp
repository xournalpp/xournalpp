...
auto ImageOpenDlg::show(GtkWindow* win, Settings* settings, bool localOnly, bool* attach) -> GFile* {
    ...
    if (!settings->getLastImagePath().empty()) {
        gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(dialog), Util::toGFile(settings->getLastImagePath()).get(),
                                            nullptr);
    }
    ...
    if (auto folder = Util::fromGFile(
                Util::GOwned<GFile>(gtk_file_chooser_get_current_folder(GTK_FILE_CHOOSER(dialog))).get());
        !folder.empty()) {
