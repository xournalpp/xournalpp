...
void Settings::parseItem(xmlDocPtr doc, xmlNodePtr cur) {
    ...
    if (name == "pressureSensitivity") {
        ...
        this->lastSavePath = fs::u8path(value);
    } else if (name == "lastOpenPath") {
        this->lastOpenPath = fs::u8path(value);
        ...
        this->latexSettings.globalTemplatePath = fs::u8path(value);
...
auto Settings::load() -> bool {
    ...
    if (ustring_view(cur->name) != "settings"_xmlsv) {
