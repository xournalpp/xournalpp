...
RenameLayerDialog::RenameLayerDialog(GladeSearchpath* gladeSearchPath, UndoRedoHandler* undo, LayerController* lc,
    ...
    gtk_entry_buffer_set_text(gtk_entry_get_buffer(GTK_ENTRY(get("layerNameEntry"))), lc->getCurrentLayerName().c_str(),
                              lc->getCurrentLayerName().size());
...
void RenameLayerDialog::renameSuccessful(GtkButton* bttn, RenameLayerDialog* rld) {
    std::string newName = gtk_entry_buffer_get_text(gtk_entry_get_buffer(GTK_ENTRY(rld->get("layerNameEntry"))));
