...
public:
    ShowLayerEntry(LayerController* lc, Layer::Index id);
    ...
    ShowLayerEntry(ShowLayerEntry&& e);
    ShowLayerEntry& operator=(ShowLayerEntry&&);
    ~ShowLayerEntry();
    xoj::util::WidgetSPtr checkButton;
    gulong callbackHandler;
};

ToolPageLayer::ShowLayerEntry::ShowLayerEntry(LayerController* lc, Layer::Index id):
...
ToolPageLayer::ShowLayerEntry::ShowLayerEntry(ShowLayerEntry&& e):
        checkButton(std::move(e.checkButton)), callbackHandler(std::exchange(e.callbackHandler, 0U)) {}

auto ToolPageLayer::ShowLayerEntry::operator=(ShowLayerEntry&& e) -> ShowLayerEntry& {
    checkButton = std::move(e.checkButton);
    callbackHandler = std::exchange(e.callbackHandler, 0U);
    return *this;
}

ToolPageLayer::ShowLayerEntry::~ShowLayerEntry() {
...
static std::pair<GtkWidget*, std::vector<ToolPageLayer::ShowLayerEntry>> createGrid(LayerController* lc) {
     ...
     *
     * To circumvent this, we make our own GAction/RadioButton interactions
     */
    GtkRadioButton* group = nullptr;
    auto createLayerRadioButton = [&group](const std::string& layerName, Layer::Index id) -> GtkWidget* {
    ...
    auto createLayerRadioButton = [&actionName, &group](const std::string& layerName, Layer::Index id) -> GtkWidget* {
...
void ToolPageLayer::setupLayersGrid() {
    auto [grid, entries] = createGrid(lc);
