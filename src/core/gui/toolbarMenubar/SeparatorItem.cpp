...
auto SeparatorItem::createItem(bool horizontal) -> xoj::util::WidgetSPtr {
    return nullptr;  // xoj::util::WidgetSPtr(GTK_WIDGET(gtk_separator_tool_item_new()), xoj::util::adopt);
}

auto SeparatorItem::getToolDisplayName() const -> std::string { return _("Separator"); }

auto SeparatorItem::getNewToolIcon() const -> GtkWidget* { return gtk_image_new_from_icon_name(ICON_NAME); }
