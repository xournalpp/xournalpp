...
auto InputEvents::translateEvent(GdkEvent* sourceEvent, Settings* settings, GtkWidget* referenceWidget, int nbPress)
    ...
    xoj::util::Point<double> relativeOffset(0, 0);
    if (GTK_IS_SCROLLABLE(referenceWidget)) {
        relativeOffset = {gtk_adjustment_get_value(gtk_scrollable_get_hadjustment(GTK_SCROLLABLE(referenceWidget))),
                          gtk_adjustment_get_value(gtk_scrollable_get_vadjustment(GTK_SCROLLABLE(referenceWidget)))};
    }
    ...
    if (xoj::util::Point<double> surfCoord; gdk_event_get_position(sourceEvent, &surfCoord.x, &surfCoord.y)) {
        eventMould.absolute = xoj::util::gtk::gdkSurfaceToWidgetCoordinates(surfCoord, referenceWidget);
        eventMould.relative = eventMould.absolute + relativeOffset;
    } else {
    ...
    if (sourceEventType == GDK_MOTION_NOTIFY) {
        ...
        if (backlog) {
            ...
            for (auto* ev = backlog; ev < backlog + backlogSize; ev++, it++) {
                it->absolute = xoj::util::gtk::gdkSurfaceToWidgetCoordinates(
                        {ev->axes[GDK_AXIS_X], ev->axes[GDK_AXIS_Y]}, referenceWidget);
                it->relative = it->absolute + relativeOffset;
