...
XojPageView::XojPageView(XournalView* xournal, const PageRef& page):
        ...
        oldtext(nullptr),
        pixelCache(Range(0., 0., page->getWidth(), page->getHeight()), xournal->getDpiScaleFactor(),
                   [this](auto& node) {
                       this->scheduleMakeTile(node.getArea(), node.getDepth());
                       return false;
                   }) {
...
void XojPageView::scheduleMakeTile(const Range& area, unsigned depth) {
    {
        xoj::view::QuadCache::TileInfo ti{area, depth};
        ...
        if (std::find_if(this->tilesToRender.begin(), this->tilesToRender.end(), [&](const auto& t) {
            ...
            this->tilesToRender.emplace_back(xoj::view::QuadCache::TileInfo{area, depth});
        }
    }
...
auto XojPageView::paintPage(cairo_t* cr, GdkRectangle* rect) -> bool {
...

    {
        ...
        //     return true;
        // }
        this->pixelCache.paintAndPreload(cr, .3);
