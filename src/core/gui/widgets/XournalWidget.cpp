...
auto gtk_xournal_new(XournalView* view, InputContext* inputContext, GtkAdjustment* vadj, GtkAdjustment* hadj)
    ...
    xoj->popovers = new std::vector<GtkPopover*>();
...
static void gtk_xournal_size_allocate(GtkWidget* widget, int width, int height, int baseline) {
    GtkXournal* xournal = GTK_XOURNAL(widget);
    gtk_adjustment_set_page_size(xournal->hadjustment, width);
    gtk_adjustment_set_page_size(xournal->vadjustment, height);

    for (auto* p: *xournal->popovers) {
...
void gtk_xournal_add_popover(GtkWidget* xournal, GtkPopover* popover) {
    GTK_XOURNAL(xournal)->popovers->emplace_back(popover);
    gtk_widget_set_parent(GTK_WIDGET(popover), xournal);
    gtk_widget_queue_allocate(xournal);  // make sure the popover have been "gtk_popover_present()'ed".
}

void gtk_xournal_remove_popover(GtkWidget* xournal, GtkPopover* popover) {
    std::erase(*GTK_XOURNAL(xournal)->popovers, popover);
