...
ActionBackwardCompatibilityLayer::ActionBackwardCompatibilityLayer(Control* ctrl): control(ctrl) {}

void ActionBackwardCompatibilityLayer::actionPerformed(std::string_view type, bool enabled) {
    ...
    if (auto it = STATELESS_ACTION_MAP.find(type); it != STATELESS_ACTION_MAP.end()) {
        ...
        if (type == "ACTION_TOGGLE_PAIRS_PARITY") {
            int offset = control->getSettings()->getPairsOffset();
