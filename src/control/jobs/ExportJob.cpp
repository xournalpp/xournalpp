...
bool ExportJob::freeSurface(int id) {
	...
	if (this->type == EXPORT_FORMAT_PNG) {
		char * path = NULL;
		if (id == -1) {
			path = g_strdup_printf("%s%c%s.png", this->folder.c_str(), G_DIR_SEPARATOR, this->filename.c_str());
		} else {
			path = g_strdup_printf("%s%c%s%i.png", this->folder.c_str(), G_DIR_SEPARATOR, this->filename.c_str(), id);
		}
		...
		for (int i = 0; i < count; i++) {
			...
			if (selectedPages[i]) {
				...
				if (page->getBackgroundType() == BACKGROUND_TYPE_PDF) {
					...
					// TODO: pdf is written as image to the SVN surface!!
