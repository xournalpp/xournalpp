...
auto Rectangle<T>::intersects(const Rectangle<T>& other, Rectangle<T>* dest) const -> bool {
    ...
    T destX = std::max(this->x, other.x);
    T destY = std::max(this->y, other.y);
    T destW = std::min(this->x + this->width, other.x + other.width) - destX;
    T destH = std::min(this->y + this->height, other.y + other.height) - destY;
