...
def docs_for_functions(file_name:str, functions: dict[str, str]):
    ...
    repl_comment_prefix = re.compile(r'^\s\*')
...
def fmt_luaLS_decl(file, function_name:str, comments:list[str] = [], params:list[str] = []):
    '''
...
if __name__ == "__main__":
    # argument handling
    if len(sys.argv) > 4:
        print(f"Usage: {sys.argv[0]} [path to the file to inspect]")
        quit(-1)
    else:
        if len(sys.argv) >= 1:
            if len(sys.argv) >= 2:
